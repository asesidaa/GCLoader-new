...
namespace gc::absolute_judgement
{
    namespace
    {
        constexpr ULONGLONG kPeriodicSnapshotCadenceMilliseconds = 5'000;
        constexpr std::size_t kTimingTraceEntriesPerLine = 16;
        ...
        AbsoluteJudgementTransientPublicationCounts
        SubtractTransientPublications(
            const AbsoluteJudgementTransientPublicationCounts& value,
            const AbsoluteJudgementTransientPublicationCounts& baseline)
            noexcept
        {
            return {
                .arrange = SubtractMonotonic(
                    value.arrange, baseline.arrange),
...
#define GC_SUBTRACT_COUNTER(field) \
    result.field = SubtractMonotonic(value.field, baseline.field)
            ...
            result.last_overload_drop_sequence =
                value.last_overload_drop_sequence;
            ...
            GC_SUBTRACT_COUNTER(periodic_snapshot_records);
            GC_SUBTRACT_COUNTER(periodic_snapshot_drops);
            GC_SUBTRACT_COUNTER(timing_trace_records);
            GC_SUBTRACT_COUNTER(timing_trace_drops);
...
#undef GC_SUBTRACT_COUNTER
        ...
        const char* ScopeKindName(
            const AbsoluteJudgementScopeKind kind) noexcept
        {
            switch (kind)
            {
...
#undef GC_FATAL_PREDICATE
        ...
        void AppendTimingTraceEntry(
            std::string& message,
            const AbsoluteJudgementTimingTraceEntry& entry)
        {
            std::format_to(
                std::back_inserter(message),
                " entry_begin scope_id={} kind={} native_ms={}",
                entry.scope_id,
                ScopeKindName(entry.kind),
                entry.native_ms);
            AppendQueries(message, "scope_", entry.queries);
            AppendTimingGradeObservations(
                message, "scope_", entry.timing_grades);
            AppendScoreDeltas(message, "scope_", entry.score_deltas);
            message.append(" entry_end");
        }
            ...
            const AbsoluteJudgementCounterSnapshot& counters)
        {
            ...
            std::format_to(
                std::back_inserter(message),
                " {}periodic_snapshot_records={}"
                " {}periodic_snapshot_drops={}"
                " {}timing_trace_records={} {}timing_trace_drops={}"
                ...
                prefix,
                counters.periodic_snapshot_records,
                prefix,
                counters.periodic_snapshot_drops,
                prefix,
                counters.timing_trace_records,
                prefix,
                counters.timing_trace_drops,
                prefix,
                counters.timing_trace_records,
                prefix,
                counters.timing_trace_drops,
    ...
    AbsoluteJudgementDiagnostics::SnapshotCounters() const noexcept
    {
        return {
            ...
            .periodic_snapshot_records = stage_.periodic_snapshot_records,
            .periodic_snapshot_drops = stage_.periodic_snapshot_drops,
            .timing_trace_records = stage_.timing_trace_records,
            .timing_trace_drops = stage_.timing_trace_drops,
            .score_observation_read_failures =
    ...
    void AbsoluteJudgementDiagnostics::ResetStageState() noexcept
    {
        ...
        periodic_diagnostics_size_ = 0;
        periodic_diagnostics_drops_ = 0;
        periodic_diagnostics_window_ = 0;
        timing_trace_size_ = 0;
        timing_trace_drops_since_flush_ = 0;
        ...
        next_periodic_snapshot_tick_ms_ =
            GetTickCount64() + kPeriodicSnapshotCadenceMilliseconds;
    }
    ...
    bool AbsoluteJudgementDiagnostics::PeriodicDiagnosticsDue() noexcept
    {
        const auto now = GetTickCount64();
        if (now < next_periodic_snapshot_tick_ms_)
        {
            return false;
        }
        next_periodic_snapshot_tick_ms_ =
            now + kPeriodicSnapshotCadenceMilliseconds;
        return true;
    }

    void AbsoluteJudgementDiagnostics::CapturePeriodicDiagnostics(
        const AbsoluteJudgementRuntimeSnapshot& runtime) noexcept
    {
        CapturePeriodicDiagnostics(runtime, false);
    }

    void AbsoluteJudgementDiagnostics::CapturePeriodicDiagnostics(
        const AbsoluteJudgementRuntimeSnapshot& runtime,
        const bool final_window) noexcept
    {
        const bool retained =
            periodic_diagnostics_size_ < periodic_diagnostics_.size();
        if (retained)
        {
            IncrementSaturating(stage_.periodic_snapshot_records);
        }
        else
        {
            IncrementSaturating(periodic_diagnostics_drops_);
            IncrementSaturating(stage_.periodic_snapshot_drops);
        }
        IncrementSaturating(periodic_diagnostics_window_);

        const auto cumulative = SnapshotCounters();
        const auto interval = SnapshotIntervalCounters(cumulative);
        if (retained)
        {
            periodic_diagnostics_[periodic_diagnostics_size_] = {
                .runtime = runtime,
                .interval = interval,
                .final_window = final_window,
            };
            ++periodic_diagnostics_size_;
        }
        last_periodic_snapshot_ = cumulative;
        ResetIntervalMaxima();
    }

    void AbsoluteJudgementDiagnostics::FlushPeriodicDiagnostics(
        const std::uint64_t stage_generation) noexcept
    {
        for (std::size_t index = 0;
             index < periodic_diagnostics_size_;
             ++index)
        {
            const auto& record = periodic_diagnostics_[index];
            auto message = std::format(
                "AbsoluteJudgement: deferred-five-second-summary"
                " stage_generation={} window={}/{} final_window={}"
                " retained_windows={} dropped_windows={}",
                stage_generation,
                index + 1,
                periodic_diagnostics_window_,
                record.final_window ? 1 : 0,
                periodic_diagnostics_size_,
                periodic_diagnostics_drops_);
            AppendRuntime(message, record.runtime);
            AppendCounters(message, "interval_", record.interval);
            PLOG_INFO << message;
        }
        periodic_diagnostics_size_ = 0;
        periodic_diagnostics_drops_ = 0;
    ...
    void AbsoluteJudgementDiagnostics::FlushTimingTrace(
        const std::uint64_t stage_generation) noexcept
    {
        if (timing_trace_size_ == 0 &&
            timing_trace_drops_since_flush_ == 0)
        {
            return;
        }

        const auto part_count = (std::max)(
            std::size_t{1},
            (timing_trace_size_ + kTimingTraceEntriesPerLine - 1) /
            kTimingTraceEntriesPerLine);
        for (std::size_t part = 0; part < part_count; ++part)
        {
            const auto begin = part * kTimingTraceEntriesPerLine;
            const auto end = (std::min)(
                begin + kTimingTraceEntriesPerLine, timing_trace_size_);
            auto message = std::format(
                "AbsoluteJudgement: scope-trace reason=stage-end"
                " stage_generation={} window=1 part={}/{} entries_total={}"
                " entries_in_part={} dropped_since_last_flush={}",
                stage_generation,
                part + 1,
                part_count,
                timing_trace_size_,
                end - begin,
                timing_trace_drops_since_flush_);
            for (auto index = begin; index < end; ++index)
            {
                AppendTimingTraceEntry(message, timing_trace_[index]);
            }
            PLOG_INFO << message;
        }
        timing_trace_size_ = 0;
        ...
        const AbsoluteJudgementSemanticStageEndRecord& record) noexcept
    {
        ...
        CapturePeriodicDiagnostics(record.runtime, true);
        FlushPeriodicDiagnostics(record.loader_stage_generation);
        FlushTimingTrace(record.loader_stage_generation);
        const auto cumulative = SnapshotCounters();
        auto message = std::format(
    ...
    {
        const auto timing_grade_records = (std::min)(
        ...
        const bool timing_trace_relevant =
            record.timing_grades.calls != 0 ||
            record.score_deltas.miss != 0 ||
            record.score_deltas.good != 0 ||
            record.score_deltas.cool != 0 ||
            record.score_deltas.great != 0;
        if (timing_trace_relevant)
        {
            if (timing_trace_size_ < timing_trace_.size())
            {
                timing_trace_[timing_trace_size_] = {
                    .scope_id = record.scope_id,
                    .kind = record.kind,
                    .native_ms = record.native_ms,
                    .queries = record.queries,
                    .timing_grades = record.timing_grades,
                    .score_deltas = record.score_deltas,
                };
                ++timing_trace_size_;
                IncrementSaturating(stage_.timing_trace_records);
            }
            else
            {
                IncrementSaturating(timing_trace_drops_since_flush_);
                IncrementSaturating(stage_.timing_trace_drops);
            }
        }
    }
