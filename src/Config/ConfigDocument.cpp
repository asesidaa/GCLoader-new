...
namespace gc::config
{
    namespace
    {
        ...
        std::expected<void, ConfigDocumentLoadError> ValidateObsoleteSyntax(
            const toml::table& syntax)
        {
            ...
            {
                return std::unexpected(ConfigDocumentLoadError{
                    .code = ConfigDocumentLoadErrorCode::obsolete_schema,
                    .message = std::string{kObsoleteInputSchemaError},
                });
            }
            ...
            {
                return std::unexpected(ConfigDocumentLoadError{
                    .code = ConfigDocumentLoadErrorCode::unsupported_schema,
                    .message =
            ...
            if (schema_version->get() != kInputSchemaVersion)
            {
                return std::unexpected(ConfigDocumentLoadError{
                    .code = ConfigDocumentLoadErrorCode::unsupported_schema,
                    .message = "Unsupported input_schema_version; expected 2",
                });
            }
                ...
                experimental->contains(kObsoleteFramerateBoolean))
            {
                return std::unexpected(ConfigDocumentLoadError{
                    ...
                    "timer_patches is not supported; replace it with "
                    "target_fps = 60 through 500",
                });
            }
            return {};
        }
    ...
    std::expected<ParsedConfigDocument, ConfigDocumentLoadError>
    ParseConfigDocument(std::string_view text) noexcept
    {
        try
        {
            ...
            {
                return std::unexpected(ConfigDocumentLoadError{
                    .code = ConfigDocumentLoadErrorCode::toml_syntax,
                    .message = std::move(syntax.error()),
                });
            }
            if (auto obsolete = ValidateObsoleteSyntax(*syntax); !obsolete)
            {
                return std::unexpected(std::move(obsolete.error()));
            }
            auto registry_migration = MigrateLegacyRegistryPaths(*syntax);
            if (!registry_migration)
            {
                return std::unexpected(ConfigDocumentLoadError{
                    .code = ConfigDocumentLoadErrorCode::strict_shape,
                    .message = std::move(registry_migration.error()),
                });
            }
            ...
            if (!audio_migration)
            {
                return std::unexpected(ConfigDocumentLoadError{
                    .code = ConfigDocumentLoadErrorCode::strict_shape,
                    .message = std::move(audio_migration.error()),
                });
            }
            ...
            auto parsed =
                rfl::toml::read<ConfigDocument, rfl::NoExtraFields>(
                    canonical_text.str());
            if (!parsed)
            {
                return std::unexpected(ConfigDocumentLoadError{
        ...
        catch (const std::exception& error)
        {
            return std::unexpected(ConfigDocumentLoadError{
                .code = ConfigDocumentLoadErrorCode::strict_shape,
                .message =
                "Config document parsing failed: " +
        ...
        catch (...)
        {
            return std::unexpected(ConfigDocumentLoadError{
                .code = ConfigDocumentLoadErrorCode::strict_shape,
                .message = "Config document parsing failed unexpectedly",
            });
        }
    }

    std::expected<std::string, ConfigDocumentLoadError>
    SerializeConfigDocument(const ConfigDocument& document) noexcept
    {
        ...
        {
            return std::unexpected(ConfigDocumentLoadError{
                .code = ConfigDocumentLoadErrorCode::serialization,
                .message =
                "Config document serialization failed: " +
                std::string{error.what()},
            });
        }
        catch (...)
        {
            return std::unexpected(ConfigDocumentLoadError{
                .code = ConfigDocumentLoadErrorCode::serialization,
                .message = "Config document serialization failed unexpectedly",
            });
        }
    }
        ...
        const AtomicConfigWriteActions& actions) noexcept
    {
        ...
        ConfigPersistenceStage stage = ConfigPersistenceStage::serialize;
        try
        {
            ...
            {
                return std::unexpected(ConfigPersistenceError{
                    .stage = ConfigPersistenceStage::serialize,
                    .message = std::move(serialized.error().message),
                });
            }
            stage = ConfigPersistenceStage::temporary_write;
    ...
    {
        auto result =
            WriteConfigDocumentAtomically(config_path, config, actions);
        if (!result)
        {
