...
static_assert(
    ...
    std::numeric_limits<std::int64_t>::max() /
        static_cast<std::int64_t>(kNanosecondsPerMillisecond));

[[nodiscard]] ExactOutputClockResult Result(
    const ExactClockStatus status,
    const std::uint64_t endpoint_generation,
    const std::uint64_t submitted_output_tail = 0,
    const std::uint64_t anchor_sequence = 0,
    const std::optional<std::uint64_t> anchor_position =
...
[[nodiscard]] std::int64_t MultimediaDeltaNanoseconds(
    const std::uint32_t event_time_ms,
    const std::uint64_t anchor_system_time_ns) noexcept {
    const auto anchor_time_ms = static_cast<std::uint32_t>(
        anchor_system_time_ns / kNanosecondsPerMillisecond);
    const auto anchor_remainder_ns = static_cast<std::int64_t>(
        anchor_system_time_ns % kNanosecondsPerMillisecond);
    const auto delta_bits = static_cast<std::uint32_t>(
        event_time_ms - anchor_time_ms);
    const auto delta_ms = static_cast<std::int64_t>(
        std::bit_cast<std::int32_t>(delta_bits));
    return delta_ms *
            static_cast<std::int64_t>(kNanosecondsPerMillisecond) -
        anchor_remainder_ns;
}
...
ExactAsioClock::ExactAsioClock(
    ...
    const std::uint32_t output_latency_frames,
...
std::shared_ptr<ExactAsioClock> ExactAsioClock::Create(
    ...
    if (endpoint_generation == 0 ||
        output_sample_rate != kSupportedOutputSampleRate ||
    ...
    auto* provider = new (std::nothrow) ExactAsioClock(
        endpoint_generation,
        output_sample_rate,
        qpc_frequency,
        period_frames,
...
bool ExactAsioClock::Publish(const ExactAsioAnchor& anchor) noexcept {
    ...
    const bool decreasing_identity = writer_has_anchor_ &&
        (anchor.sequence <= writer_previous_.sequence ||
         anchor.presented_output_frame <=
             writer_previous_.presented_output_frame ||
    ...
    slot.presented_output_frame.store(
        anchor.presented_output_frame, kExactClockAtomicOrder);
    slot.system_time_ns.store(
        anchor.system_time_ns, kExactClockAtomicOrder);
...
bool ExactAsioClock::ReadStable(
    const std::uint64_t publication,
    ExactAsioAnchor* const anchor) const noexcept {
    if (anchor == nullptr ||
        publication >
            (std::numeric_limits<std::uint64_t>::max() - 2) / 2) {
        return false;
    ...
    for (int attempt = 0; attempt < kStableReadAttempts; ++attempt) {
        ...
        const ExactAsioAnchor candidate{
            ...
            .presented_output_frame =
                slot.presented_output_frame.load(kExactClockAtomicOrder),
            .system_time_ns =
                slot.system_time_ns.load(kExactClockAtomicOrder),
...
ExactOutputClockResult ExactAsioClock::Resolve(
    ...
    for (std::uint64_t offset = 0; offset < available; ++offset) {
        ...
        const auto delta_ns = MultimediaDeltaNanoseconds(
            timestamp.multimedia_time_ms, anchor.system_time_ns);
        if (delta_ns >= 0) {
            if (unstable) {
                return CountResult(Result(
        ...
        return CountResult(Result(
            ...
            selected
                ? std::optional<std::uint64_t>(
                      selected->presented_output_frame)
                : std::nullopt));
    }
    if (!selected.has_value()) {
        ...
        if (published > capacity_ && oldest.has_value() &&
            MultimediaDeltaNanoseconds(
                timestamp.multimedia_time_ms,
                oldest->system_time_ns) < 0) {
            return CountResult(Result(
                ExactClockStatus::HistoryLost,
                endpoint_generation_,
                latest_tail,
                oldest->sequence,
                oldest->presented_output_frame));
        }
        return CountResult(Result(
    ...
    if (anchor.presented_output_frame >
            static_cast<std::uint64_t>(
                std::numeric_limits<std::int64_t>::max()) ||
        latest_tail > static_cast<std::uint64_t>(
            std::numeric_limits<std::int64_t>::max())) {
        return CountResult(Result(
            ExactClockStatus::Discontinuous,
            endpoint_generation_,
    ...
    const auto delta_seconds = gc::timing::CheckedRational::Create(
        selected_delta_ns, kNanosecondsPerSecond);
    ...
    const auto output_frame = output_offset
        ? gc::timing::CheckedRational::Whole(
              static_cast<std::int64_t>(anchor.presented_output_frame))
              .Add(*output_offset)
        : std::expected<gc::timing::CheckedRational,
                        gc::timing::RationalError>(
              std::unexpected(gc::timing::RationalError::Overflow));
    if (!delta_seconds || !output_offset || !output_frame ||
    ...
    if (latest_tail == 0 ||
        output_frame->Compare(gc::timing::CheckedRational::Whole(
            static_cast<std::int64_t>(latest_tail))) >= 0) {
        return CountResult(Result(
            ExactClockStatus::Pending,
            endpoint_generation_,
            latest_tail,
    ...
    return CountResult({
        ...
        .anchor_endpoint_position = anchor.presented_output_frame,
    });
}
