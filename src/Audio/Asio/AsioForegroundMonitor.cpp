...
{
    PublishForeground(QueryCurrentForeground());
}

void AsioForegroundMonitor::PublishForeground(
    const bool foreground) noexcept
{
    const bool previous = foreground_.exchange(
        foreground, std::memory_order_acq_rel);
    if (previous != foreground && change_event_ != nullptr)
    {
...
bool AsioForegroundMonitor::QueryCurrentForeground() const noexcept
{
