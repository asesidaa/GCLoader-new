...
namespace gc::audio
{
    ...
    public:
        ...
        static std::shared_ptr<ExactWasapiClock> Create(std::uint64_t endpoint_generation,
                                                        std::uint32_t output_sample_rate, std::uint64_t clock_frequency,
    ...
    private:
        ...
        ExactWasapiClock(std::uint64_t endpoint_generation, std::uint32_t output_sample_rate,
                         std::uint64_t clock_frequency, std::int64_t qpc_frequency, std::uint32_t period_frames,
                         std::size_t capacity, std::unique_ptr<Slot[]> slots) noexcept;

        [[nodiscard]] bool ReadStable(std::uint64_t publication, ExactWasapiAnchor* anchor) const noexcept;
