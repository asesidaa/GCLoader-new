...
std::optional<int> CompareQpc(
    const RawQpcParts& raw,
    std::int64_t raw_frequency,
    std::uint64_t qpc_100ns) noexcept {
    const auto anchor_whole =
        qpc_100ns / kReferenceTimePerSecond;
    if (raw.whole_seconds < 0) {
        return -1;
    }

    const auto raw_whole =
        static_cast<std::uint64_t>(raw.whole_seconds);
    if (raw_whole != anchor_whole) {
        return raw_whole < anchor_whole ? -1 : 1;
    }

    const auto raw_fraction = gc::timing::CheckedRational::Create(
        static_cast<std::int64_t>(raw.remainder),
...
std::optional<gc::timing::CheckedRational> QpcDeltaSeconds(
    const RawQpcParts& raw,
    std::int64_t raw_frequency,
    std::uint64_t anchor_qpc_100ns) noexcept {
    const auto anchor_whole =
        anchor_qpc_100ns / kReferenceTimePerSecond;
    if (raw.whole_seconds < 0 ||
        static_cast<std::uint64_t>(raw.whole_seconds) < anchor_whole) {
        return std::nullopt;
    }

    const auto whole_delta =
        static_cast<std::uint64_t>(raw.whole_seconds) - anchor_whole;
    if (whole_delta > static_cast<std::uint64_t>(
    ...
    const auto with_raw_fraction =
        gc::timing::CheckedRational::Whole(
            static_cast<std::int64_t>(whole_delta))
            .Add(*raw_fraction);
    ...
    if (!delta.has_value() ||
        delta->Compare(gc::timing::CheckedRational::Whole(0)) < 0) {
        return std::nullopt;
    }
    return *delta;
}
...
bool SameMapping(
    const EndpointClockMapping& left,
    const EndpointClockMapping& right) noexcept {
    return left.origin_position == right.origin_position &&
        left.clock_frequency == right.clock_frequency &&
...
ExactWasapiClock::ExactWasapiClock(
    std::uint64_t endpoint_generation,
...
std::shared_ptr<ExactWasapiClock> ExactWasapiClock::Create(
    std::uint64_t endpoint_generation,
    std::uint32_t output_sample_rate,
    std::uint64_t clock_frequency,
    std::int64_t qpc_frequency,
    std::uint32_t period_frames) noexcept {
    if (endpoint_generation == 0 || output_sample_rate == 0 ||
        clock_frequency == 0 ||
        clock_frequency > static_cast<std::uint64_t>(
            std::numeric_limits<std::int64_t>::max()) ||
    ...
    const auto retention_frames =
        kRetentionSeconds * output_sample_rate;
    ...
    auto slots = std::unique_ptr<Slot[]>(
        new (std::nothrow) Slot[static_cast<std::size_t>(capacity)]);
    if (slots == nullptr) {
        return nullptr;
    }

    auto* provider = new (std::nothrow) ExactWasapiClock(
        endpoint_generation,
        output_sample_rate,
        clock_frequency,
        qpc_frequency,
...
void ExactWasapiClock::Publish(
    const ExactWasapiAnchor& anchor) noexcept {
    ...
    const bool invalid_mapping =
        ...
        anchor.endpoint_position - anchor.mapping.origin_position >
            static_cast<std::uint64_t>(
                std::numeric_limits<std::int64_t>::max()) ||
        anchor.mapping.origin_output_frame >
            static_cast<std::uint64_t>(
                std::numeric_limits<std::int64_t>::max()) ||
        anchor.submitted_output_tail >
            static_cast<std::uint64_t>(
                std::numeric_limits<std::int64_t>::max());
    const bool decreasing_identity = writer_has_anchor_ &&
        (anchor.sequence <= writer_previous_.sequence ||
         anchor.endpoint_position < writer_previous_.endpoint_position ||
         anchor.qpc_100ns < writer_previous_.qpc_100ns ||
         anchor.submitted_output_tail <
            writer_previous_.submitted_output_tail ||
         !SameMapping(anchor.mapping, writer_previous_.mapping));
    if (anchor.sequence == 0 ||
        anchor.endpoint_generation != endpoint_generation_ ||
        invalid_mapping || decreasing_identity ||
        writer_publication_count_ >
    ...
    slot.endpoint_generation.store(
        anchor.endpoint_generation, kExactClockAtomicOrder);
    slot.endpoint_position.store(
        anchor.endpoint_position, kExactClockAtomicOrder);
    slot.qpc_100ns.store(anchor.qpc_100ns, kExactClockAtomicOrder);
    slot.mapping_origin_position.store(
        anchor.mapping.origin_position, kExactClockAtomicOrder);
    slot.mapping_clock_frequency.store(
        anchor.mapping.clock_frequency, kExactClockAtomicOrder);
    slot.mapping_origin_output_frame.store(
        anchor.mapping.origin_output_frame, kExactClockAtomicOrder);
    slot.mapping_output_sample_rate.store(
        anchor.mapping.output_sample_rate, kExactClockAtomicOrder);
    slot.submitted_output_tail.store(
        anchor.submitted_output_tail, kExactClockAtomicOrder);
    slot.version.store(writing_version + 1, kExactClockAtomicOrder);
    published_count_.store(
        publication + 1, kExactClockAtomicOrder);
...
bool ExactWasapiClock::ReadStable(
    ...
    for (int attempt = 0; attempt < kStableReadAttempts; ++attempt) {
        ...
        ExactWasapiAnchor candidate{
            ...
            slot.qpc_100ns.load(kExactClockAtomicOrder),
            {
                ...
                slot.mapping_origin_output_frame.load(
                    kExactClockAtomicOrder),
                slot.mapping_output_sample_rate.load(
                    kExactClockAtomicOrder),
            },
...
ExactOutputClockResult ExactWasapiClock::ResolveQpc(
    ...
    const auto available = std::min<std::uint64_t>(
        published, static_cast<std::uint64_t>(capacity_));
    ...
    for (std::uint64_t offset = 0; offset < available; ++offset) {
        ...
        const auto comparison = CompareQpc(
            raw, qpc_frequency_, anchor.qpc_100ns);
    ...
    if (!selected.has_value()) {
        ...
        if (published > capacity_ && oldest.has_value()) {
            const auto comparison = CompareQpc(
                raw, qpc_frequency_, oldest->qpc_100ns);
    ...
    const auto delta_seconds = QpcDeltaSeconds(
        raw, qpc_frequency_, anchor.qpc_100ns);
    ...
    const auto clock_delta = delta_seconds->Multiply(
        static_cast<std::int64_t>(clock_frequency_), 1);
    const auto mapped_endpoint_delta =
        anchor.endpoint_position - anchor.mapping.origin_position;
    if (!clock_delta.has_value() ||
        mapped_endpoint_delta > static_cast<std::uint64_t>(
            std::numeric_limits<std::int64_t>::max())) {
        return Result(
            ExactClockStatus::Discontinuous,
            endpoint_generation_,
    ...
    const auto endpoint_offset =
        gc::timing::CheckedRational::Whole(
            static_cast<std::int64_t>(mapped_endpoint_delta))
            .Add(*clock_delta);
    ...
    const auto output_offset = endpoint_offset->Multiply(
        ...
        anchor.mapping.clock_frequency);
    ...
    if (anchor.submitted_output_tail == 0 ||
        output_frame->Compare(gc::timing::CheckedRational::Whole(
            static_cast<std::int64_t>(
                anchor.submitted_output_tail))) >= 0) {
        return Result(
            ExactClockStatus::Pending,
            endpoint_generation_,
            anchor.submitted_output_tail,
