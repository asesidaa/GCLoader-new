...
namespace gc::audio
{
    ...
    std::expected<void, AudioRuntimeError> PrepareAndPublishAudioRuntime(
        ...
        try {
            ...
            const auto text = audio_config_text(backend, buffer_ms);
            emit_info(production_platform_actions(), text.c_str());
    ...
    bool IsAsioRuntimePublished() noexcept {
        const auto* state = g_production_detour_state.load(std::memory_order_acquire);
    ...
    std::expected<void, hooking::HookError> AddPublishedAudioRuntimeHook(
        ...
        auto* state = g_production_detour_state.load(std::memory_order_acquire);
        ...
        if (!state) return {}; // DirectSound retains the original backend.
    ...
    void ReleaseAudioRuntimeAtOrdinaryAsioClose(std::uintptr_t site_rva) noexcept {
        auto* owner = g_production_detour_state.exchange(nullptr, std::memory_order_acq_rel);
