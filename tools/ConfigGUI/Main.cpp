...
namespace
{
    ...
    public:
        GuiInputContext(
            gc::config::ConfigDocument& config,
    ...
    void DrawRegistry(gc::config::ConfigDocument& config, bool& dirty)
    {
    ...
    void DrawLogging(gc::config::ConfigDocument& config, bool& dirty)
    {
    ...
    std::expected<gc::config::ParsedConfigDocument, std::string> LoadConfig(
...
int main(int argc, char** argv)
{
    ...
    gc::config::ConfigDocument config = std::move(loaded->document);
    ...
    while (host.PumpMessages())
    {
        ...
        if (auto saved = audio_worker.TakeSave())
        {
            ...
            else
            {
                ...
                {
                    config = std::move(refreshed->document);
                    editor = InputEditorModel(config.controller());
                    input.RefreshDevices();
                    audio_editor.NotifyConfigReloaded();
                    dirty = false;
                    save_status = "Configuration saved and reloaded.";
                }
            }
        }
        ...
        config.controller = editor.config();
        const auto compiled = gc::config::ConfigCompiler::Compile(config);
        if (!compiled)
        {
            ...
            for (const auto& error : compiled.error())
            {
                ...
                ImGui::BulletText(
                    "%s: %s", path.c_str(), error.message.c_str());
            }
        }
