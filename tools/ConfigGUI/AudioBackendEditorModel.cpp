...
namespace
{
    ...
    std::expected<void, std::string> ValidateCapabilityForSettings(
        const gc::audio::AsioSettings& settings,
        const gc::audio::AsioCapabilityReport& report)
    {
        const auto requested_frames = settings.buffer_frames();
        const auto requested_base = settings.output_base_channel();
        if (!EqualInsensitive(
...
void AudioBackendEditorModel::SetBackend(
    gc::audio::AudioBackend backend) noexcept
{
    ...
    try
    {
            ...
            &compiled->audio().selection()))
        {
            ...
            const auto validated =
                ValidateCapabilityForSettings(*asio, *result);
