...
**Architecture:** Each voice owns a custom zero-input/one-output miniaudio node attached to a no-device engine endpoint plus a preinitialized `ma_data_converter`. Mixer and voices share the engine-state owner, which can also retain an aliasing allocation-callback owner, while each voice retains shared snapshot/timeline owners. The node callback uses raw pointers from those stable owners, captures one immutable `AudioSnapshot` view, fills fixed input scratch, converts PCM16/PCM24 mono/stereo to 44.1 kHz stereo float, and publishes epoch-cumulative cursor spans. Native-rate voices take miniaudio's format/channels-only path; only 22.05/48 kHz sources use the linear resampler.
...
- Preserve the borrowed raw-callback `Create` overload for existing/default callers. The owning overload accepts `std::shared_ptr<const ma_allocation_callbacks>` and stores it in engine state before `ma_engine_init`; an aliasing owner therefore remains alive through voice node/converter teardown and final `ma_engine_uninit`.
...
10. Destroy the public mixer before its voice, then stop/destroy the voice safely and prove all callback allocations are released exactly once by the last shared engine owner. For the owning callback overload, drop every external alias owner before public mixer destruction, require the owner to remain live while the voice exists, then prove node/converter/final-engine frees complete before owner release with no later callback.
...
`MiniaudioMixer` stores `shared_ptr<MiniaudioMixerState>` and copies that owner into each voice. `MiniaudioMixerState` stores the owning callback `shared_ptr` before engine initialization when the owning overload is used; the borrowed raw overload leaves that field empty and retains its existing caller-lifetime contract. `CreateVoice` accepts and stores shared snapshot/timeline owners. The callback takes raw pointers with `.get()` only while the state-held shared owners remain unchanged. All last-owner destruction remains off-render. Plan 06 must allocate buffers/timelines as shared owners and pass them directly to this API.
