...
- [ ] Implement copy quads with `D3DFVF_XYZRHW | D3DFVF_TEX1`, `DrawPrimitiveUP`, point sampling, clamped addressing, half-pixel-correct coordinates, and source UVs derived from texture dimensions. Force solid fill and disable culling, user clip planes, coordinate wrapping, texture transforms, blend, depth, stencil, fog, scissor, and sRGB sample/write; route stage-zero texture color/alpha to current and enable all color channels for the copy.
...
- [ ] Keep the scene/depth bound during physical rendering and native/no-depth bound during native rendering. After restoring game pipeline state, explicitly disable and validate native Z enable, Z write, and stencil before handing control back.
