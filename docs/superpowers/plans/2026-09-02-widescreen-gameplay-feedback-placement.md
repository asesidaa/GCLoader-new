...
HUD content at native 720 x 1280 scale, place each ordinary combo counter on
...
the ordinary CHAIN label and digits, flushing native batches at each boundary.
...
- Keep the stage background, perspective track, chart objects, hit effects,
  player markers, opponent judgements, and milestone combo celebration
  unchanged.
- Move the ordinary combo label/digits only. Entry 0 uses the right 720-pixel
  viewport, entry 1 uses the left viewport, and any unexpected entry falls
  back to the centered viewport.
...
| Combo end | `0x001E4558` | `C7 45 CC 00 00 00 00` | Restore the centered HUD viewport after the ordinary digit call. |
...
## Task 3: Install guarded combo and gameplay-feedback hooks
...
- [x] Extend `WidescreenContractSite`, the contract arrays, ABI metadata, and
  stable site names. The final transaction contains 34 executable requests
...
- [x] Amend the design with the distinction among centered native-texture 2D,
  ...
  viewports.
- [x] Record the ordinary combo hook window and explain why the milestone
  counter remains centered.
